  ...
  const getPlaylistDetail = async () => {
    try {
      ...
      if (response.status === 404) {
        setError('Playlist not found. It might have been deleted or made private.');
        setIsLoading(false);
        return;
      }

      if (response.status === 403) {
        setError('You do not have permission to access this playlist.');
        setIsLoading(false);
        return;
      }
      ...
      if (error.response) {
        ...
        if (error.response.status === 401) {
          // Token expired - you might want to refresh the token here
            ...
            <div className='playlist_main' >
              <div className='playlist_main_header' >
                ...
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  ...
                  <h2>{data?.name}</h2>
                  <div style={{ display: "flex", gap: ".6rem", alignItems: "center", height: '50px' }} >

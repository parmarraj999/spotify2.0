...
function PlaylistPage() {
  ...
  const [play,setPlay] = useState([playlistData])

  const checkMyPlaylistData = () => {
    console.log(playlistData)
    const foundPlaylist = play?.find(data => data.playlistId === id)
    if (foundPlaylist) {
      console.log(foundPlaylist)
      setPlaylistDetail(foundPlaylist);
  ...
  return (
    <div className='playlist_page_container' >
      <div className='playlist_page_header' >
        <img src={playlistDetail?.playlistImage} />
      ...
      <div className='playlist_page_list_container' >
        ...
        </div>
        {
          data.map((data) => {
            return (
              <SongBar data={data} playlistName={playlistDetail?.playlistName} />
            )
          })
        }
      </div>
